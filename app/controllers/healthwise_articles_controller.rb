class HealthwiseArticlesController < ApplicationController
  ...
  def show
    ...
    if @healthwise_article.send("#{I18n.locale}_translated") == false
      ...
      if @healthwise_article.updated_at < Time.now - 7.days
        if @healthwise_article.type == "article"
          @healthwise_article.send("#{I18n.locale}_json") = fetch_article(@healthwise_article)
        else
          @healthwise_article.send("#{I18n.locale}_json") = fetch_topic(@healthwise_article)
        end
      end
  ...
  def create
    ...
    if @healthwise_article.type == "Article"
      ...
      @healthwise_article.languages = @fetch_article_languages(@healthwise_article)
      ...
      @healthwise_article.languages.each do |l|   # ["en-us", "vi-us"]
        ...
        @healthwise_article.send("#{set_ci_locale(l)}_json") = response
        @healthwise_article.send("#{set_ci_locale(l)}_title") = response["data"]["title"]["consumer"]
      end
  ...
  private
    ...
    def fetch_article_languages(article)
      token = fetch_hw_token
      url = ENV['HEALTHWISE_CONTENT_URL'] + "/articles/#{article.hwid}"
